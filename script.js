document.addEventListener('DOMContentLoaded', function () {
    ...
    // Ensure carousel slide width is calculated correctly
    carouselSlide.style.transform = `translateX(${-slideWidth * counter}px)`;
  
    // Move carousel slide automatically every 7 seconds (adjust as needed)
    setInterval(() => {
      counter++;
      carouselSlide.style.transition = 'transform 1s ease-in-out';
      carouselSlide.style.transform = `translateX(${-slideWidth * counter}px)`;
    }, 7000); // Change interval time to 7000 milliseconds (7 seconds)
  
    // Reset carousel position after last slide
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    emailField.addEventListener("input", function() {
      ...
      if (!trimmedValue.endsWith("@gmail.com")) {
        if (warningMessage) {
          return; // Exit if warning message already exists
        }
